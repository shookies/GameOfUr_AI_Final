...
class Board:
    ...
    def __run_game(self):
        ...
        # self._turn, self._opponent = ('W', 'B') if white_roll > black_roll else ('B', 'W')
        self.__turn = 'B'
        self.__opponent = 'W' if self.__turn == 'B' else 'B'
        ...
        while not winner:
            ...
            if current_player == self.__player_B:
                ...
                GUI.update_msg(
                    'White rolled ' + str(sum(self.__current_dice_roll)))
                time.sleep(random.uniform(0.1, 2))  # simulate thinking
            ...
            while self.__is_rosette(*new_position):
                # print("loop 2")
                GUI.draw_board(self, self.__turn, (-1, -1))
                if current_player == self.__player_B:
                    chosen_action = GUI.get_action(self)
                else:
...
if __name__ == "__main__":
    ...
    count_b = 0
    num_of_games = 100
    learning_agent = Agents.DeepQAgent('B', learning_on=False, network_path='C:\\Users\\Edan\\Desktop\\Studies\\AI\\Project\\final_project_ai\\game\\weights')
    last_ten_w = 0
    last_ten_b = 0
    games_played = 0
    for i in range(num_of_games):
        ...
        if games_played % 10 == 0:
            print("in the last 10 games black won {} times and white won {} times".format(last_ten_b, last_ten_w))
            games_played, last_ten_w, last_ten_b = (0, 0, 0)
            if b._Board__player_B.learning_on:
